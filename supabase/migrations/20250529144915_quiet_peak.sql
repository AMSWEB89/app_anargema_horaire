...
-- Fonction pour marquer tous les employés comme absents
CREATE OR REPLACE FUNCTION mark_all_employees_absent_for_date(target_date date)
RETURNS void AS $$
BEGIN
  -- Marquer tous les employés comme absents pour la date spécifiée
  INSERT INTO employee_attendance (employee_id, date, is_present)
  SELECT id, target_date, false
  FROM employees
  ON CONFLICT (employee_id, date) DO NOTHING;
END;
$$ LANGUAGE plpgsql;