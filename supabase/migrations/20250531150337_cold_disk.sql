...
CREATE TABLE IF NOT EXISTS public.employee_schedules (
    ...
    CONSTRAINT valid_times CHECK (
        (
            -- Si les deux pauses sont NULL, c'est valide
            (pause_debut IS NULL AND pause_fin IS NULL)
            OR
            -- Si seulement pause_debut est rempli
            (pause_debut IS NOT NULL AND pause_fin IS NULL AND pause_debut >= heure_debut)
            OR
            -- Si seulement pause_fin est rempli
            (pause_debut IS NULL AND pause_fin IS NOT NULL AND 
        ...
        AND
        -- Si heure_fin est remplie, elle doit être après heure_debut
        (heure_fin IS NULL OR heure_debut < heure_fin)
    )
);
